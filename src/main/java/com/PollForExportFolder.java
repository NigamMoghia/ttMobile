...
public class PollForExportFolder {
    ...
    public void  findLatestManifest(String type) throws Exception {
        ...
        switch (type){
            case "PR":
                ...
                query = "SELECT TOP 1 * FROM ED_ADMIN.BUILD_MANIFEST\n" +
                        ...
                        "WHERE BUILD_NAME LIKE '%PUBLISH READY%' AND BUILD_NAME not like '%ADHOC%' )\n" +
        ...
        GlobalVariables.pr_exportFolder = exportFolder_pr;
        GlobalVariables.pr_exportDate = exportBenginDate_pr ;
        GlobalVariables.pr_manifestID = manifestID;
        GlobalVariables.pr_exportBenginDate = exportBenginDate_pr;
        GlobalVariables.pr_exportEndDate = exportEndDate_pr;
        con.close();
...
//verify the UptoDate all_and_PR Site
    public boolean verifyWebPage(String env,String exportType) throws Exception{
    	...
    	String exportRegionUrl=env + " " + exportType;
    	url = getEnvironmentSite(env,exportType);
        String status = null;
        //make xpath what needs to be verified
        if(exportType.equalsIgnoreCase("UpToDate")){
            exportType ="PR";
        }
        ...
        exportRegionUpdate = "<a style=\"color:blue\" href=\""+url+"\">"+exportRegionUrl+"</a>";
        if(res & result){
            result=true;
            status = "pass";
        }
